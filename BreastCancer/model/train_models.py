...
'''
        ...
        # For XGBoost, save the booster parameters
        import json
        booster_config = json.loads(model.get_booster().save_config())
        booster_dump = model.get_booster().get_dump(dump_format='json')
...
classes_ = np.array({model.classes_.tolist()})
booster_config = {json.dumps(booster_config)}
booster_dump = {booster_dump}

# Note: Full XGBoost prediction requires the XGBoost library
# This is a simplified version for demonstration
# For production use, consider saving as .json and loading with xgboost.Booster
...
def predict_proba(X):
    ...
    try:
        ...
        booster = xgb.Booster()
        booster.load_model(bytearray(json.dumps(booster_config), 'utf-8'))
...
# Alternative: Save booster to JSON file
booster_json = json.dumps(booster_config)
'''
    
    with open(f'model/{model_type}.py', 'w') as f:
        f.write(code)
    print(f"Model saved to model/{model_type}.py")
